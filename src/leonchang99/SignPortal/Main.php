        ...
        if($event->getBlock()->getID() == 323 || $event->getBlock()->getID() == 63 || $event->getBlock()->getID() == 68){
            ...
            if($sign[0]=='[WORLD]'){
                if(empty($sign[1]) !== true){
                    $mapname = $sign[1];
                    $event->getPlayer()->sendMessage("§6§lFluddelDudel §7> §r§fWelt '".$mapname."' wird vorbereitet");
                    //Prevents most crashes
                    $p = $this->api->player->get("username");
                    $this->api->console->run("cb",$p);
                }
            }
        }
    }
                ...
                if($event->getPlayer()->hasPermission("signportal.create")){
                    ...
                    if(empty($sign[1]) !==true){
                        ...
                        if(Server::getInstance()->loadLevel($sign[1])!==false){
                            ...
                            $event->getPlayer()->sendMessage("§6§lFluddelDudel §7> §r§fPortal zu '".$sign[1]."' erstellt");
                            return true;
                        }
                        $event->getPlayer()->sendMessage("§6§lFluddelDudel §7> §r§fWelt '".$sign[1]."' existiert nicht");
                        //Server::getInstance()->broadcastMessage("f4");
                        $event->setLine(0,"[BROKEN]");
                        return false;
                    }
                    $event->getPlayer()->sendMessage("§6§lFluddelDudel §7> §r§fWelt name nicht gestzt");
                    //Server::getInstance()->broadcastMessage("f3");
                    $event->setLine(0,"[BROKEN]");
                    return false;
                }
            $event->getPlayer()->sendMessage("§6§lFluddelDudel §7> §r§fDu brauchst die benötigte berechtigung um ein Portal zu erstellen");
